...
async function sluts() {
    ...
    while (document.getElementById('partyTime').checked) {
        if (last.innerHTML.search('a href="/assets/') !== -1 && last.innerHTML.search('spoil/default.jpg') === -1 && !(last.classList.contains("deleted"))) {
            ...
            if (lastpostHTML.includes('.webm') || lastpostHTML.includes('.mp4')) {
                ...
                if (!userbackgroundwebm.firstChild.src.includes(lastpostHTML) || (userbackgroundwebm.firstChild.muted != document.getElementById("partymute").checked)) {
            ...
            if (lastpostHTML.includes(".gif") || lastpostHTML.includes(".png") || lastpostHTML.includes(".jpg")) {
                if (!userbackgroundpng.firstChild.wholeText.includes(lastpostHTML)) {
                    userbackgroundpng.firstChild.replaceWith(`#user-background {background: url(${lastpostHTML}) no-repeat fixed center; background-size:cover;background-size: cover;}`);
                }
...
function setup() {
    ...
    document.getElementById("partyTime").onclick = function() {
        if (this.checked) { //Starts background changing.
            ...
            console.log("Setting slut.party value to false");
    ...
    document.getElementById("partymute").onclick = function() {
        if (this.checked) {
            ...
            localStorage.setItem("slut.mute", false);
        }
    }
