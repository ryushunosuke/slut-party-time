// ==UserScript==
// @name     Slut party time
// @include https://meguca.org/a/*
// @version  0.3.2
// @grant none
// @icon data:image/png;base64,
// @run-at document-end
// ==/UserScript==


function sleep(ms) {
  return new Promise(resolve => setTimeout(resolve, ms));
}

async function sluts() {
    var thread = document.getElementById('thread-container');
    var last = thread.lastElementChild;
  while (true) { //Infinite loop. Can be done much more efficiently by making each and every post signal a function that is listening
    //Which then can call what's below here to change the background.
    if (last.innerHTML.search('a href="/assets/') !== -1 && last.innerHTML.search('spoil/default.jpg') === -1){
      //Using the same background container since it can also hold images and not require you to set a custom background
      //May or may not have stolen the idea from the other guca
      var userbackground = document.getElementById('user-background');
      var lastpostHTML;
      lastpostHTML = last.innerHTML.slice(last.innerHTML.search('a href="/assets/'), last.innerHTML.length);
      lastpostHTML = lastpostHTML.slice(8, lastpostHTML.search('" download='));
      if (lastpostHTML.includes('.webm')|| lastpostHTML.includes('.mp4')) {
        // Removes the previous media that was being displayed
        if (userbackground.firstChild !== null){ 
          userbackground.removeChild(userbackground.firstChild);
        }
       //Removes the background image that was set by a previous post. You can comment this out if you don't mind seeing the image behind the webm.
       if (userbackground.firstChild === null) {
          userbackground.appendChild(document.createElement("video"));
          userbackground.firstChild.replaceWith(``); 
        }
        //If the background is not the same, replaces it.
        if (!userbackground.firstChild.src.includes(lastpostHTML)) {
          userbackground.firstChild.muted = true;
          userbackground.firstChild.src = lastpostHTML;
          userbackground.firstChild.autoplay = true;
          userbackground.firstChild.loop = true;
          userbackground.firstChild.load();
        }
      }
      //Checks if it's an image
      if (lastpostHTML.includes(".gif") || lastpostHTML.includes(".png") || lastpostHTML.includes(".jpg")){
      if(userbackground.lastChild === null)       
        if (userbackground.lastChild !== null) {
          userbackground.removeChild(userbackground.firstChild); //Removes the webm container which seems to have a priority over images/gifs
        }//Checks if it's a duplicate
        if (!userbackground.firstChild.wholeText.includes(lastpostHTML)) {
          userbackground.appendChild(document.createElement("img"));
          userbackground.firstChild.replaceWith(``);
        }
      }
    }
      await sleep(40); // Can be set to a higher value to change the minimum duration of a bg staying the same
      if (last.nextSibling !== null) {
        last = last.nextSibling;
      }
  }
}


try{
  sluts();
}
catch(err){
  console.log(err.message);
}